 ...
 */
public class AssignmentPerformanceImporter extends PerformanceClass {
    /**
     * method that queries assignment, quarter or year results.
     *
     * @param connection     connection to database
     * @param pForm          contains all input data returned from view
    ...
    List<String> queryResults(final Connection connection, final PerformanceForm pForm) {
        ...
        try {
            ...
            String queryString = "";
            queryString = formAssignmentQuery();
            String[] options = createOptions(pForm);
            queryString = "select avg(R.RESULT), " + options[2] + queryString + options[0] + options[1];
    ...
    /**
     * method that queries assignment results.
     *
     * @return query body before where clause
     */
    private String formAssignmentQuery() {
        return " from SCHOOLS S\n"
                ...
                + "join ASSIGNMENTS A on R.ASSIGNMENTID=A.ASSIGNMENTID and A.SGID = SG.ID\n";
    }
}
